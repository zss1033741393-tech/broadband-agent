...
`planning` 产出的方案是分段 Markdown，每段有 `**启用**: true/false` 头。Orchestrator 按段落标题匹配 Provisioning 实例：
...
**识别特征**：Planning 回复内容是一句自然语言追问 + 已识别槽位摘要，**没有**分段方案 Markdown（缺少 `## WIFI 仿真方案` 等标题）。
...
**识别特征**：回复含 5 段标题（`## WIFI 仿真方案` / `## 差异化承载方案` / `## CEI 配置方案` / `## 故障诊断方案` / `## 远程闭环处置方案`），且每段带 `**启用**: true/false` 头。

**Orchestrator 动作**：
1. 把方案 Markdown 呈现给用户
2. 末尾附一段明确的确认问句，例如：
   > "以上为初步方案，其中 **WIFI 仿真 / 差异化承载 / 体验保障链** 三项将被执行。是否确认执行？（回复『执行』开始；也可以提出修改建议）"
3. **停下等待用户明确回复**：
