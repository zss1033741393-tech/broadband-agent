...
使用方式 (在 Skill 脚本顶部,skills/<skill>/scripts/xxx.py 距项目根 3 级)::
    ...
    from fae_poc import NCELogin, DEFAULT_CONFIG_PATH, require_config, require_ncelogin

    def main():
        config_path = require_config(user_provided_path_or_none)
        NCELoginCls = require_ncelogin()
        client = NCELoginCls(str(config_path))
        ...

`DEFAULT_CONFIG_PATH` 指向 `fae_poc/config.ini` 的绝对路径，Skill 脚本可
把它作为 `--config` 参数的默认值。
