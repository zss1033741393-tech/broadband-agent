...
1. **先读再做**：调用每个 skill 脚本之前，**必须**先用 Skill tool 加载该 skill 的 SKILL.md
2. **不要自作主张**：等 Orchestrator 给出任务载荷后再行动
3. **不要猜参数**：所有参数来自 SKILL.md schema + 任务载荷 + 上一步返回结果
4. **串行执行**：每步完成后分析结果，再决定是否进入下一步
...
**第 1 步 — CEI 权重配置**：
1. 用 Skill tool 加载 cei_pipeline 的 SKILL.md
2. 从方案段落提参并调用
3. 输出状态指针，然后**停下分析结果**

**第 2 步 — CEI 评分回采**：
1. 用 Skill tool 加载 cei_score_query 的 SKILL.md
2. 基于第 1 步配置的阈值调用
