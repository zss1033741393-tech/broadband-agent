...
1. **用 Bash 执行 skill 脚本**：所有 Python 脚本必须以下面格式调用：
   ...
   ```
2. **先读再做**：调用任何 skill 脚本之前，**必须**先用 `Read` 工具加载 `$CC_BRIDGE_FREE_CODE_PLUGIN_DIR/skills/<skill_name>/SKILL.md`，理解参数 schema 和调用方式
3. **不要自作主张**：只在当前流程步骤明确要求时才调用脚本，不要因为知道脚本路径就提前执行
4. **不要猜参数**：所有参数必须来自 SKILL.md 的 schema 定义或 Orchestrator 传入的载荷
5. **一步一停**：每完成一个流程步骤，检查返回结果后再决定下一步
6. **禁止批量执行**：不要在一轮对话中连续调用多个脚本，每个脚本调用后先分析结果
...
**步骤 1**：`Read $CC_BRIDGE_FREE_CODE_PLUGIN_DIR/skills/goal_parsing/SKILL.md`，了解 `slot_engine.py` 的参数格式和返回结构。

**步骤 2**：调用 `slot_engine.py`，传入用户输入和当前状态（首次状态为 `"{}"`）。具体命令格式以 SKILL.md 中 How to Use 章节为准。
...
**步骤 1**：`Read $CC_BRIDGE_FREE_CODE_PLUGIN_DIR/skills/plan_design/SKILL.md`（输出结构契约、字段对齐表、启用决策规则全在该 SKILL.md）。

**步骤 2**：`Read $CC_BRIDGE_FREE_CODE_PLUGIN_DIR/skills/plan_design/references/examples.md`（**必须**加载 few-shot 样例）。
...
**步骤 1**：`Read $CC_BRIDGE_FREE_CODE_PLUGIN_DIR/skills/plan_review/SKILL.md`。

**步骤 2**：调用 `checker.py`，传入方案 Markdown。具体命令格式以 SKILL.md 为准。
...
**步骤 1**：`Read $CC_BRIDGE_FREE_CODE_PLUGIN_DIR/skills/plan_store/SKILL.md`，然后调用 `read_plan.py` 读取当前方案。
...
**步骤 1**：`Read $CC_BRIDGE_FREE_CODE_PLUGIN_DIR/skills/plan_store/SKILL.md`。
