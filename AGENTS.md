...
- `script_path`：脚本文件名，如 `"run_phase.py"`
- `execute`：设为 `true` 执行脚本，`false` 读取脚本内容
- `args`：字符串数组，传给脚本的命令行参数，如 `['{"phase_id":1,"table_level":"day","steps":[...]}']`
- `timeout`：超时秒数，默认 60

### 正确示例
get_skill_script(skill_name="insight_query", script_path="run_phase.py", execute=true, args=['{"phase_id":1,"phase_name":"...","table_level":"day","steps":[...]}'])
get_skill_script(skill_name="goal_parsing", script_path="slot_engine.py", execute=true, args=["<user_input>", "<state_json>"])

### 禁止的方式
❌ bash tool：uv run python skills/insight_query/scripts/run_phase.py '{"phase_id":...}'
