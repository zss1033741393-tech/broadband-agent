...
2. 调用 `get_skill_script("report_generation", "render_report.py", execute=True, args=["--insight", "<insight_json_string>"])`
3. 将返回的 `stdout` Markdown 内容直接展示给用户

**综合目标模式**（来自 solution_generation 输出）：
1. 构建上下文 JSON：`{"title": "...", "profile": {...}, "configs": {...}, "verification": {...}}`
2. 调用 `get_skill_script("report_generation", "render_report.py", execute=True, args=["<context_json_string>"])`
3. 将返回的 `stdout` Markdown 内容直接展示给用户

**禁止**：不要基于模板自行生成报告文本，必须通过脚本渲染。

> ⚠️ **CRITICAL — 输出规则**：`render_report.py` 的 `stdout` 即为完整报告，**必须原样复制到最终回答**。任何基于报告内容的二次改写、摘要、重排均被视为违规，不论改写后格式是否更美观。
...
**数据洞察模式调用**:
```
get_skill_script("report_generation", "render_report.py", execute=True, args=["--insight", "<mock_query.py 的完整 JSON 输出字符串>"])
```

**综合目标模式调用**:
```
get_skill_script("report_generation", "render_report.py", execute=True, args=["{\"title\": \"...\", \"profile\": {...}, \"configs\": {...}}"])
```

**输出**: 结构化 Markdown 报告（stdout 即最终产物，必须原样输出）
