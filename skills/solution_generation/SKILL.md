...
## How to Use
1. 接收 slot_filling 输出的画像 JSON
2. **判断输入模式**：
   - 若 JSON 中有 `"mode": "data_insight"` → 调用脚本中的 `render_from_insight(profile_json)`
   - 否则 → 调用脚本中的 `render_all(profile_json)`
3. 调用 `get_skill_script("solution_generation", "render.py")` 并传入画像 JSON
4. 脚本根据画像参数渲染四类配置模板：
...
5. 将四份配置（及 optimization_focus，若有）整合为完整方案展示给用户
