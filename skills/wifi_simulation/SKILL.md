...
3. `args` 必须是 `List[str]`，列表仅含一个 JSON 字符串元素
   > ⚠️ **常见错误**：`args` 的元素必须是 **字符串**，不得是 Python dict  
   > ❌ `args=[{}]`、`args=[{"preset":"三居室"}]` — 元素为 dict，pydantic 会报 `Input should be a valid string`  
   > ✅ `args=['{}']`、`args=['{"preset":"三居室"}']` — 元素为 JSON 字符串（先 `json.dumps` 再放入列表）
