...
def _make_next_func(return_value):
    """构造一个返回固定值的同步 next_func（模拟 Agno 工具链）"""
...
class TestOutputSinkHook:
    ...
    @pytest.mark.asyncio
    async def test_saves_intent_output(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        ctx = _FakeRunContext(session_id="sess-abc")
        payload = {"complete": True, "missing_fields": []}

        await output_sink_hook(
    ...
    @pytest.mark.asyncio
    async def test_saves_configs_output(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
    ...
    @pytest.mark.asyncio
    async def test_all_stage_scripts_mapped(self, tmp_path, monkeypatch):
        """四个阶段脚本都能正确写入对应文件名"""
        monkeypatch.chdir(tmp_path)
    ...
    @pytest.mark.asyncio
    async def test_passthrough_non_skill_script_call(self, tmp_path, monkeypatch):
        """非 get_skill_script 调用不写文件，返回值原样透传"""
        monkeypatch.chdir(tmp_path)
        ctx = _FakeRunContext(session_id="sess-pass")
        sentinel = {"key": "value"}

        result = await output_sink_hook(
    ...
    @pytest.mark.asyncio
    async def test_execute_false_no_write(self, tmp_path, monkeypatch):
        """execute=False 时不写文件"""
        monkeypatch.chdir(tmp_path)
    ...
    @pytest.mark.asyncio
    async def test_unknown_script_no_write(self, tmp_path, monkeypatch):
        """未知脚本名不写文件，不抛异常"""
        monkeypatch.chdir(tmp_path)
    ...
    @pytest.mark.asyncio
    async def test_invalid_json_no_exception(self, tmp_path, monkeypatch):
        """stdout JSON 解析失败时跳过写入，不抛异常"""
        monkeypatch.chdir(tmp_path)
    ...
    @pytest.mark.asyncio
    async def test_no_run_context_falls_back_to_unknown(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_returns_original_result_unchanged(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_overwrites_on_retry(self, tmp_path, monkeypatch):
